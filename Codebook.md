...
Variables that begin with "t" are time domain signals, while those that begin with "f" are frequency domain signals derived from a a Fast Fourier Transform (FFT).
...
Only the variables with "mean" or "std" in their name were retained in the cleaned dataset  
